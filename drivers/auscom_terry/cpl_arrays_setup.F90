...
! A> atm==>ice 
!                                       
! (1) 2m air temp (K)                      		tair1 
! (2) 10m 'zonal' wind speed (m/s)         		uwnd1 
! (3) 10m 'meridional' wind speed (m/s)     		vwnd1 
! (4) shortwave radiation (down, J/m^2)     		swflx1
! (5) longwave radiation  (down, J/m^2)     		lwflx1
! (6) 2m air specific humidity (kg/kg)      		qair1 
! (7) rainfall rate     (kg/m^2/s)        		prec1 
! (8) snowfall rate     (kg/m^2/s)                      prec1
! (9) pressure            (Pa)                		press1
! (10)runof               (kg/m^2/s)          		runof1 
!
! B> ocn==>ice 
!                          
...
real(kind=dbl_kind), dimension(:,:,:), allocatable :: &   !from atm
   ...
   ,runof0, press0   
real(kind=dbl_kind), dimension(:,:,:), allocatable :: &   !from atm
    tair1, swflx1, lwflx1, uwnd1, vwnd1, qair1, rain1, snow1 & !(for ice)
   ,runof1, press1                                        !(for ocn)
real(kind=dbl_kind), dimension(:,:,:), allocatable :: &   !we now use newt press        
    runof, press                                          !and runoff
