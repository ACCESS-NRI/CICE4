...
!!!use ice_dyn_evp,   only : sicemass  !toatl mass of ice and snow (in m)
...
!===============================================================================
subroutine get_time0_a2i_fields(fname)

implicit none

character*(*), intent(in) :: fname
integer(kind=int_kind) :: ncid_a2i
logical :: dbug

dbug = .true.
if ( file_exist(fname) ) then 
  if (my_task == 0) write(il_out,*) '(get_time0_a2i_fields) reading in a2i fields......' 
  call ice_open_nc(fname, ncid_a2i) 
  call ice_read_nc(ncid_a2i, 1, 'TAIR',  tair0,  dbug)
  call ice_read_nc(ncid_a2i, 1, 'SWFLX', swflx0, dbug)
  call ice_read_nc(ncid_a2i, 1, 'LWFLX', lwflx0, dbug)
  call ice_read_nc(ncid_a2i, 1, 'UATM',  uwnd0,  dbug)
  call ice_read_nc(ncid_a2i, 1, 'VATM',  vwnd0,  dbug)
  call ice_read_nc(ncid_a2i, 1, 'QAIR',  qair0,  dbug)
  call ice_read_nc(ncid_a2i, 1, 'RAIN',  rain0,  dbug)
  call ice_read_nc(ncid_a2i, 1, 'SNOW',  snow0,  dbug)
  call ice_read_nc(ncid_a2i, 1, 'PRESS', press0, dbug)
  call ice_read_nc(ncid_a2i, 1, 'RUNOF', runof0, dbug)
  if (my_task == master_task) call ice_close_nc(ncid_a2i)
  runof0 = runof0 / 21600.        ! kg/m2/6h => kg/m2/s
  if (my_task == 0) write(il_out,*) '(get_time0_a2i_fields) has read in 10 a2i fields.'
else
  if (my_task == 0) write(il_out,*) 'ERROR: (get_time0_a2i_fields) not found file *** ',fname
  print *, 'CICE: (get_time0_a2i_fields_old) not found file *** ',fname
  stop 'CICE stopped -- Need time0 a2i data file.'
endif

return
end subroutine get_time0_a2i_fields
...
!===============================================================================
subroutine newt_forcing_atmo_raw(nt) 

implicit none

integer(kind=int_kind), intent(in) :: nt
real(kind=dbl_kind) :: coef0, coef1

coef1 = float((nt-1))*float(dt_cice)/float(dt_cpl_ai)
coef0 = 1. - coef1 

! --- from atm: 
tair(:,:,:)  = coef0 * tair0(:,:,:)  + coef1 * tair1(:,:,:)
fsw(:,:,:)   = coef0 * swflx0(:,:,:) + coef1 * swflx1(:,:,:)
flw(:,:,:)   = coef0 * lwflx0(:,:,:) + coef1 * lwflx1(:,:,:)
uatm(:,:,:)  = coef0 * uwnd0(:,:,:)  + coef1 * uwnd1(:,:,:)
vatm(:,:,:)  = coef0 * vwnd0(:,:,:)  + coef1 * vwnd1(:,:,:)
qa(:,:,:)    = coef0 * qair0(:,:,:)  + coef1 * qair1(:,:,:)
frain(:,:,:) = coef0 * rain0(:,:,:)  + coef1 * rain1(:,:,:)
fsnow(:,:,:) = coef0 * snow0(:,:,:)  + coef1 * snow1(:,:,:)
! -------- temporary test : --------
frain = frain * precip_factor
fsnow = fsnow * precip_factor
! ----------------------------------
press(:,:,:) = coef0 * press0(:,:,:) + coef1 * press1(:,:,:)
runof(:,:,:) = coef0 * runof0(:,:,:) + coef1 * runof1(:,:,:)

! ---
! --- from ocean: (no time interpolation needed!) 
! --- 
uocn(:,:,:) = ssuo(:,:,:)
vocn(:,:,:) = ssvo(:,:,:)
where (ssto .gt. 200)                   !make sure SST is 'all right' 
  sst(:,:,:) = ssto(:,:,:) - 273.1      !K==>C!
endwhere
!WATCH: dbl-chk land masks for possible mismatch between cice and cpl!
sss(:,:,:)     = ssso(:,:,:)
ss_tltx(:,:,:) = sslx(:,:,:)
ss_tlty(:,:,:) = ssly(:,:,:)
...
end subroutine newt_forcing_atmo_raw

!===============================================================================
subroutine update_time0_a2i_fields

! --- by the end of each cpl interval, update the starting point (time0)
! --- forcing for the next cpl interval
...
end subroutine update_time0_a2i_fields

!===============================================================================
subroutine save_time0_a2i_fields(fname, nstep)
! --- output the last a2i forcing data rcvd from cpl, to be used at the 
! --- beginning of next run (for time interpolation)
...
integer(kind=int_kind), intent(in) :: nstep
integer(kind=int_kind) :: ncid
integer(kind=int_kind) :: ll, ilout,jf
character*80 :: a2i_fields(10)
data a2i_fields/'TAIR','SWFLX','LWFLX','UATM','VATM','QAIR','RAIN','SNOW','PRESS','RUNOF'/
...
do jf = 1, 10
  ...
  if (my_task == 0) then
    call write_nc2D(ncid, trim(a2i_fields(jf)), gwork, 2, nx_global, ny_global, 1, ilout=il_out)
  endif
enddo
...
return
end subroutine save_time0_a2i_fields

!===============================================================================
subroutine save_i2a_fields(fname,nstep)
implicit none
...
return 
end subroutine save_i2a_fields

!===============================================================================
subroutine save_o2i_fields(fname,nstep)
implicit none
...
integer(kind=int_kind) :: ncid
integer(kind=int_kind) :: jf, ll, ilout
character*80 :: o2i_fields(7)
data o2i_fields/'sst_i','sss_i','ssu_i','ssv_i','sslx_i','ssly_i','pfmice_i'/
...
do jf = 1, 7
  ...
  call gather_global(gwork, vwork, master_task, distrb_info)
  if (my_task == 0) then
    call write_nc2D(ncid, trim(o2i_fields(jf)), gwork, 2, nx_global, ny_global, 1, ilout=il_out)
  endif
enddo
...
return 
end subroutine save_o2i_fields
...
do iblk = 1, nblocks
   ...
   tv_atm(:,:) = tair1(:,:,iblk) * (1.0 + d608*qair1(:,:,iblk))
   !air density 10 meter height, 'rough', 'cos of using p_surf
   d_atm(:,:) = press1(:,:,iblk) / (rdgas * tv_atm(:,:))
   !estimate of 10 meter (zlvl) pressure
   p_atm(:,:) = press1(:,:,iblk) - d_atm(:,:) * gravit * 10.0
...
if (my_task == 0) call write_nc2D(ncid, 'u_star', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, rough_mom0,   master_task, distrb_info)
!if (my_task == 0) write(62,'(10e12.4)') gwork
if (my_task == 0) call write_nc2D(ncid, 'rough_mom', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, rough_heat0,   master_task, distrb_info)
!if (my_task == 0) write(63,'(10e12.4)') gwork
if (my_task == 0) call write_nc2D(ncid, 'rough_heat', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, rough_moist0,   master_task, distrb_info)
!if (my_task == 0) write(64,'(10e12.4)') gwork
if (my_task == 0) call write_nc2D(ncid, 'rough_moist', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
...
if (my_task == 0) call write_nc2D(ncid, 'isst', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
...
call gather_global(gwork, tair1, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'tair1', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, swflx1, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'swflx1', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, lwflx1, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'lwflx1', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, uwnd1, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'uwnd1', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, vwnd1, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'vwnd1', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, qair1, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'qair1', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, rain1, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'rain1', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, snow1, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'snow1', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, runof1, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'runof1', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, press1, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'press1', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
...
if (my_task == 0) call write_nc2D(ncid, 'iostrsu', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*iostrsv, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'iostrsv', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*iorain, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'iorain', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*iosnow, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'iosnow', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*iostflx, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'iostflx', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*iohtflx, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'iohtflx', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*ioswflx, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'ioswflx', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*ioqflux, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'ioqflux', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*ioshflx, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'ioshflx', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*iolwflx, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'iolwflx', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*iorunof, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'iorunof', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*iopress, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'iopress', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, scale*ioaice,  master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'ioaice',  gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
...
if (my_task == 0) call write_nc2D(ncid, 'ssto', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, ssso, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'ssso', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, ssuo, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'ssuo', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, ssvo, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'ssvo', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, sslx, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'sslx', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, ssly, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'ssly', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, pfmice, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'pfmice', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
...
call gather_global(gwork, sst, master_task, distrb_info) 
if (my_task == 0) call write_nc2D(ncid, 'sst', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, frzmlt, master_task, distrb_info) 
if (my_task == 0) call write_nc2D(ncid, 'frzmlt', gwork, 1, nx_global,ny_global,currstep,ilout=il_out)
