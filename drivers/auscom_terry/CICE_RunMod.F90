...
!
   ...
   !--------------------------------------------------------------------
      ...
      ! model (matm).
      !
      call get_time0_a2i_fields('CICE_input/A2I_time0.nc') 
      ...
      if (use_core_runoff) call get_core_runoff('CICE_input/core_runoff_regrid.nc','runoff',1)

      time_sec = 0

      DO icpl_ai = 1, num_cpl_ai   !begin I <==> O coupling iterations
      ...
      Do icpl_io = 1, num_cpl_io   !begin I <==> O coupling iterations
        ...
        do itap = 1, num_ice_io    !ice time loop within each i2o cpl interval
          ...
          !do interpolation to update the atm forcing for each time step
          call newt_forcing_atmo_raw(itap)
        ...
        if (rtimestamp_io == runtime - 86400) then
          call save_o2i_fields('o2i.nc-dm1',time_sec)
        endif
        if (my_task == 0) then
      ...
      if (time_sec == runtime - 86400) then
        call save_i2a_fields('i2a.nc-dm1',time_sec)
      endif
      if (my_task == 0) then
      ...
      call update_time0_a2i_fields 

      if (time_sec == runtime - 86400) then
        call save_time0_a2i_fields('CICE_input/A2I_time1.nc-dm1', time_sec)
        call save_time0_i2o_fields('i2o.nc-dm1', time_sec)
        call save_u_star('u_star.nc-dm1',time_sec)
      else if (rtimestamp_io == runtime ) then
        call save_time0_a2i_fields('CICE_input/A2I_time1.nc', time_sec)
        call save_time0_i2o_fields('i2o.nc', time_sec)
        call save_u_star('u_star.nc', time_sec)
      endif
...
!
!      call save_time0_a2i_fields('CICE_input/A2I_time1.nc', stimestamp_io)
!
