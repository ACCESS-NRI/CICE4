...
!-------------------------------------------------------------------------
  ...
  allocate ( tair1(nx_block,ny_block,max_blocks));  tair1(:,:,:) = 0
  allocate (swflx1(nx_block,ny_block,max_blocks)); swflx1(:,:,:) = 0
  allocate (lwflx1(nx_block,ny_block,max_blocks)); lwflx1(:,:,:) = 0
  allocate ( uwnd1(nx_block,ny_block,max_blocks));  uwnd1(:,:,:) = 0
  allocate ( vwnd1(nx_block,ny_block,max_blocks));  vwnd1(:,:,:) = 0
  allocate ( qair1(nx_block,ny_block,max_blocks));  qair1(:,:,:) = 0
  allocate ( rain1(nx_block,ny_block,max_blocks));  rain1(:,:,:) = 0
  allocate ( snow1(nx_block,ny_block,max_blocks));  snow1(:,:,:) = 0
  allocate ( runof1(nx_block,ny_block,max_blocks)); runof1(:,:,:) = 0
  allocate ( press1(nx_block,ny_block,max_blocks)); press1(:,:,:) = 0
  !note we now use 2 time level runoff and press
...
!-------------------------------------------!
  ...
  do jf = 1, n_a2i       !10, not jpfldin, only 10 fields from cpl (atm) 
    ...
    if (jf ==  1) call scatter_global(swflx1,gwork,master_task,distrb_info, &
                                      field_loc_center, field_type_scalar)
    if (jf ==  2) call scatter_global(lwflx1,gwork,master_task,distrb_info, &
                                      field_loc_center, field_type_scalar)
    if (jf ==  3) call scatter_global(rain1, gwork,master_task,distrb_info, &
                                      field_loc_center, field_type_scalar)
    if (jf ==  4) call scatter_global(snow1, gwork,master_task,distrb_info, &
                                      field_loc_center, field_type_scalar)
    if (jf ==  5) call scatter_global(press1,gwork,master_task,distrb_info, &
                                      field_loc_center, field_type_scalar)
    if (jf ==  6) call scatter_global(runof1,gwork,master_task,distrb_info, &
                                      field_loc_center, field_type_scalar)
    if (jf ==  7) call scatter_global(tair1, gwork,master_task,distrb_info, &
                                      field_loc_center, field_type_scalar)
    if (jf ==  8) call scatter_global(qair1, gwork,master_task,distrb_info, &
                                      field_loc_center, field_type_scalar)
    if (jf ==  9) call scatter_global(uwnd1, gwork,master_task,distrb_info, &
!                                      field_loc_center, field_type_scalar)
                                      field_loc_center, field_type_vector)  !20090723
    if (jf == 10) call scatter_global(vwnd1, gwork,master_task,distrb_info, &
!                                      field_loc_center, field_type_scalar)
...
!-------------------------------!
  ...
  deallocate (tair1, swflx1, lwflx1, uwnd1, vwnd1, qair1, rain1, snow1, &
              runof1, press1)
  deallocate (core_runoff)

  deallocate (tair0, swflx0, lwflx0, uwnd0, vwnd0, qair0, rain0, snow0, &
              runof0, press0)
