...
!#endif
  ...
  allocate (co2_oi(nx_block,ny_block,max_blocks));  co2_oi(:,:,:) = 0
  allocate (co2flux_oi(nx_block,ny_block,max_blocks));  co2flux_oi(:,:,:) = 0
  ...
  allocate (ioco2 (nx_block,ny_block,max_blocks)); ioco2(:,:,:) = 0
...
! no need bcast herer, instead use nickolas' changes of using haloupdate
...
!-------------------------------------------!  
  ...
  do jf = n_a2i+1, jpfldin          !no 11-17 from ocn
    ...
    if (my_task==0 .or. ll_comparal) then
      ...
      if(ll_comparal) then
        call prism_get_proto (il_var_id_in(jf), isteps, vwork2d(l_ilo:l_ihi, l_jlo:l_jhi), ierror)
!no bcast here, instead to call haloupdate
!        gwork(l_ilo:l_ihi, l_jlo:l_jhi) = vwork2d(l_ilo:l_ihi, l_jlo:l_jhi)
!        call mpi_gatherv(vwork2d(l_ilo:l_ihi, l_jlo:l_jhi),1,sendsubarray,gwork,counts,disps,resizedrecvsubarray, &
!                     0,MPI_COMM_ICE,ierror)
!        call broadcast_array(gwork, 0)
    ...
    select case (jf)
        ...
        case (n_a2i+8)
            co2_oi(1+nghost:nx_block-nghost,1+nghost:ny_block-nghost, 1) =vwork2d
        case (n_a2i+9)
            co2flux_oi(1+nghost:nx_block-nghost,1+nghost:ny_block-nghost, 1) =vwork2d
        case default
  ...
  call ice_HaloUpdate(co2_oi, halo_info, field_loc_center, field_type_scalar)
  call ice_HaloUpdate(co2flux_oi, halo_info, field_loc_center, field_type_scalar)
...
! 
  ...
  do jf = n_i2a+1, jpfldout       !no 2-14 are for the ocn
    ...
    if (jf == n_i2a+16) vwork = scale * ioco2
    if (jf == n_i2a+17) vwork = scale * iownd
...
!-------------------------------!
  ...
  deallocate (ssto, ssso, ssuo, ssvo, sslx, ssly, pfmice, co2_oi,co2flux_oi)
  deallocate (isst)
  deallocate (iostrsu, iostrsv, iorain, iosnow, iostflx, iohtflx, ioswflx, &
              ioqflux, iolwflx, ioshflx, iorunof, iopress,ioco2,iownd)
