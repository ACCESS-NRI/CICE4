...
!co2 and wnd will not be averaged
ioco2(:,:,:)  = 200.0
!iownd(:,:,:)  = 0.0
!     write(il_out,*) "ioco2=", ioco2
!     write(il_out,*) "co20=", co20
...
ioco2(:,:,:) = 200.0 
iownd = sqrt(uwnd0*uwnd0 + vwnd0*vwnd0) 
...
if ( file_exist(fname) ) then
  ...
  call ice_read_nc(ncid_o2i, 1, 'co2_oi',   co2_oi,   dbug)
  call ice_read_nc(ncid_o2i, 1, 'co2fx_oi',   co2flux_oi,   dbug)
...
if ( file_exist(fname) ) then
  ...
  do jf = n_i2a + 1, jpfldout   !2:14
    ...
    if (jf == n_i2a+16) ioco2 = vwork
    if (jf == n_i2a+17) iownd = vwork
  enddo
...
call gather_global(gwork, scale*ioco2,  master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'ioco2',  gwork, 2, nx_global,ny_global,currstep,ilout=il_out)
...
call gather_global(gwork, co2_oi, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'co2_oi', gwork, 2, nx_global,ny_global,currstep,ilout=il_out)
call gather_global(gwork, co2flux_oi, master_task, distrb_info)
if (my_task == 0) call write_nc2D(ncid, 'co2fx_oi', gwork, 2, nx_global,ny_global,currstep,ilout=il_out)
