...
!  integer, dimension(:), allocatable :: counts, disps 
  ...
  integer(kind=int_kind),dimension(2) :: i_srbuf
...
!calculate partition using nprocsX and nprocsX
  ...
  i_srbuf(1)=l_ilo
  call mpi_gather(i_srbuf, 1, mpi_integer, vilo, 1, mpi_integer, 0, MPI_COMM_ICE, ierror)
  call broadcast_array(vilo, 0)  
  i_srbuf(1)=l_jlo
  call mpi_gather(i_srbuf, 1, mpi_integer, vjlo, 1, mpi_integer, 0, MPI_COMM_ICE, ierror)
...
#error The following code assumes that max_blocks == 1
    ...
    case ('wme_i');   um_wme(1+nghost:nx_block-nghost,1+nghost:ny_block-nghost,1) = vwork2d(:,:)
!    case ('rain_i');  um_rain(:,:,:) = vwork(:,:,:)
!    case ('snow_i');  um_snow(:,:,:) = vwork(:,:,:)
!---20100825 -- just be cauious: -------------------------
    case ('rain_i');  um_rain(1+nghost:nx_block-nghost,1+nghost:ny_block-nghost,1) = max(0.0,vwork2d(:,:))
    case ('snow_i');  um_snow(1+nghost:nx_block-nghost,1+nghost:ny_block-nghost,1) = max(0.0,vwork2d(:,:))
!---------------------------------------------------------   
